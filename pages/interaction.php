...
if ($tables_check['announcements']) {
    try {
        // Total active announcements for user role
...
if ($tables_check['announcements']) {
    try {
        $stmt = $pdo->prepare("SELECT a.*, u.username as author_name 
                               ...
                               WHERE a.is_active = 1 AND (a.target_role = ? OR a.target_role = 'all')
                            ...
                            <?php foreach ($recent_announcements as $announcement): ?>
                                <div class="item-card" onclick="window.location.href='announcements.php'">
                                    ...
                                    <div class="item-meta">
                                        ...
                                        <span><i class="fas fa-clock"></i> <?php echo date('M j, Y', strtotime($announcement['created_at'])); ?></span>
                                        <span class="item-badge badge-<?php echo $announcement['target_role']; ?>">
                                            <?php echo ucfirst($announcement['target_role']); ?>
                                        </span>
                                    </div>
                                    <p class="item-preview"><?php echo htmlspecialchars($announcement['content']); ?></p>
                                </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
